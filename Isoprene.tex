...
\label{BioIsop:intro}  
  ...
  Emission models used to derive estimates of isoprene fluxes are based on understanding the emissions from different plant species (phenotypes) in varying conditions \added[id=tb]{(see Section \ref{LR:VOCs:models})}.
  ...
  Isoprene emissions in the frequently used MEGAN model may be overestimated in Australia \added[id=tb]{(see Section \ref{LR:VOCs:Emissions})} since they are based on measurements taken from a few young trees \parencite{Winters2009} that may emit more than older trees \parencite{Emmerson2016}.
  ...
  \replaced[id=mh]{This chapter describes and implements}{In this chapter, I describe and implement} a \textit{top-down} technique using satellite measurements of HCHO to calculate surface isoprene emissions.
  ...
  In this thesis \replaced[id=mh]{the technique is applied}{I apply the technique} focusing solely on Australia for the first time.
  % Too specific
  ...
  \subsection{Aims}
    ...
    \replaced[id=mh]{Isoprene emissions are estimated}{I estimate isoprene emissions} in Australia using a top-down technique based on OMI HCHO measurements and GEOS-Chem modelled yields.
    ...
    First \replaced[id=mh]{an outline is provided showing}{I outline} why current isoprene emissions estimates are inadequate and how they can be improved.
    \replaced[id=mh]{This thesis discusses}{I discuss} literature that shows how the estimates may be too high, and describe\added[id=mh]{s} how emissions may be calculated using satellite datasets.
  ...
  \subsection{Existing emissions estimates}
    ...
    \replaced[id=tb]{The bias may be as high as a factor of 6, and is not easy to fix (see Section \ref{LR:VOCs:models}).}{Bauwens et al. (2016) showed that isoprene emissions were overestimated by up to a factor of 3 over Australia.
    Emmerson et al. (2016) suggest isoprene emission estimates are 2-6 times too high compared against available measurements of isoprene concentrations.
    ...
    They compared modelled data to campaign measurements from multiple sites over different seasons and found that scaling emissions did not universally improve model outputs globally.}
    ...
    \added[id=tb]{This wide range demonstrates the effect of unknowns and uncertainties involved in isoprene emission estimation. Their results are compared to results here in Section \ref{BioIsop:results:comparisons}}.
    In this thesis \deleted[id=mh]{I develop and analyse} a top-down emission estimate \replaced[id=mh]{is developed, analysed and}{, which is} then compared against MEGAN. 
    \replaced[id=mh]{H}{I also look at h}ow changed emissions affect modelled ozone levels over Australia \added[id=mh]{is also looked into}.
  ...
  \subsection{Top-down isoprene emissions estimates}
    ...
    \subsubsection{Bayesian inversion}
      ...
      In this work \replaced[id=mh]{the Bayesian method is not used}{I do not use the Bayesian method} due to the computational costs surpassing the resources available.
...
\section{Methods}
  ...
  \replaced[id=mh]{T}{I broadly follow t}he method of \textcite{Palmer2001} \added[id=mh]{is broadly followed} to create a biogenic isoprene emissions estimate over Australia.
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \item 
        ...
        This \replaced[id=mh]{is the}{product is our} a posteriori, and calculation details are given in Section \ref{BioIsop:method:calculation}.
      \item 
  ...
  \subsection{Masks and reprocessed satellite HCHO}
    ...
    In this thesis \replaced[id=mh]{the OMI product is used}{I use the OMI product} 
    ...
    These masks are described in Section \ref{Model:filter}, and a brief \replaced[id=tb]{overview}{recap} is provided here.
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%% <<<<>>>>
  %%% MOVED HERE FROM MODELLING
  
  \subsection{Filtering Data}
  \label{Model:filter}
  
  \deleted[id=tb]{This subsection (\ref{Model:filter}) has been moved here from Chapter \ref{Model}. I have added this to-be-removed paragraph instead of highlighting everything.}
  ...
  \begin{table} \begin{threeparttable}
      ...
      \begin{tabular}{ c c c c c c }
        \textbf{Year} & \textbf{Pixels$^a$} & \textbf{Active fires} & \textbf{Anthropogenic} & \textbf{Smoke} & \textbf{Total$^b$} 
        \\ \hline
      ...
      \begin{tablenotes}
        \small
        \item a: how many land pixels are read over Australia, after cloud fraction filtering.
        \item b: Total removed pixels accounts for overlap between filters.
      \end{tablenotes}
    ...
    \mypic{Figures/OMI_link/typical_AAODs_final.png}{%
      ...
      Black Saturday (row 2) refers to the occurrence of widespread bush fires across Victoria. %, which caused 173 deaths.
    ...
    \deleted[id=mh]{We use} NO$_2$ \added[id=mh]{is used} as a proxy for potential anthropogenic NMVOC emissions, as these could bias the inversion performed in Chapter \ref{BioIsop}.
    ...
    Using two runs of GEOS-Chem with differing isoprene emissions but otherwise identical \replaced[id=mh]{provides}{we have}:
    \begin{eqnarray}
    ...
    Between these two runs\replaced[id=mh]{ only the $E$ term is changed}{ we are only changing the $E$ term}.
    ...
    \begin{table}\begin{threeparttable}
      ...
      If $\hat{S}$ sits outside the 800-4600 range then\deleted[id=mh]{ we remove} that grid square day \added[id=mh]{is removed} from both $S$ and subsequent a posteriori emission calculations.
      ...
      \begin{table} \begin{threeparttable}
        ...
        \mypic{Figures/OMI_link/Filters/smearing_nox_200501.png}{%
          ...
          Due to the logarithmic Y scale\replaced[id=mh]{ only the positive direction of standard deviations is shown}{ we only show the positive direction of standard deviations} for unfiltered data.
  ...
  \subsection{GEOS-Chem emissions}
    ...
    In this work MEGAN (version 2.1) is run as a module within GEOS-Chem (version 10.01) \added{(see Section \ref{Model:GC:MEGAN})}.
  ...
  \subsection{Relationship between isoprene emissions and formaldehyde}
    ...
    \end{equation}
    By combining Equations \ref{BioIsop:method:slope:eqn_steady_state]}, \ref{BioIsop:method:slope:eqn_loss}, and \ref{BioIsop:method:slope:eqn_prod}, \replaced[id=mh]{$\Omega$ can be related to}{we can relate $\Omega$ to} precursor emissions:
  ...
  \subsection{Modelled background HCHO}
    ...
    Since variation in HCHO columns is assumed to only depend on isoprene emissions, \replaced[id=mh]{the}{our} background term is theoretically identical to the simulated HCHO without isoprene emissions.
...
\section{Results}
  ...
  \subsection{A posteriori emissions}
    ...
    \added[id=tb]{Figure \ref{BioIsop:results:emissions:fig_tga_comparison_map} shows how the yearly decrease is distributed spatially over Australia, with $\apri$ and $\apost$ in \tgpyr ~calculated as a multi-year mean.
    ...
    \deleted[id=tb]{Figure \ref{BioIsop:results:emissions:fig_tga_comparison_map} shows how this decrease is distributed spatially, with $\apri$ and $\apost$ in \tgpyr ~calculated as a multi-year mean.
    Across all of Australia large reductions of total emissions are seen using the new top-down estimate.}
    
    \subsubsection{Comparison to other estimates}
    \label{BioIsop:results:comparisons}
    
    %Chapter 3 must include a comparative discussion of the method used in this thesis and the method of Bauwens et al. (2016), and how the results of the two studies are influenced by their choice of methodology. Similarly, the range of values for Australian isoprene emissions (both bottom-up and top-down) given in Table 3.1 should be discussed in more detail. What is it about the different emission models and inverse approaches  that lead to such a large range of values? Can the results of this work be understood simply in terms of the way the methodologies are applied, or has something genuinely new been learnt?
      
      \added[id=tb]{Table \ref{BioIsop:results:emissions:tab_emissions_Tg} provides a brief comparison of isoprene emissions estimated in this chapter of 39.2\tgpyr ~(bottom-up: from GEOS-Chem running MEGAN) and 21\tgpyr ~(top-down) with recent Australian isoprene emission estimates performed elsewhere.
      ...
      \deleted{When comparing total a priori emissions summed over Australia to the a posteriori calculated using the top-down inversion, a decrease of $\sim{46}\%$ from 39\tgpyr ~to 21\tgpyr is apparent.
      ...
      \added{Table \ref{BioIsop:results:emissions:tab_emissions_Tg} shows total a posteriori emissions summed over Australia are lower than the a priori by $\sim{46}\%$, reducing from 39\tgpyr ~to 21\tgpyr.}
      \replaced[id=mh]{The a posteriori estimated here to be 21\tgpyr}{Our a posteriori estimate of 21\tgpyr} ~suggests isoprene emissions may be lower than all prior bottom up estimates\replaced{. This}{, and} is close to the lowest \replaced{bottom-up}{top-down} estimate of \textcite{Bauwens2016} of 26\tgpyr.
      \added[id=tb]{There are several major differences between work performed in this thesis and work done by \textcite{Bauwens2016}. 
      %Here the GEOS-Chem CTM with BVOCs determined by MEGAN is used to determine a priori emissions and the steady state relationship between HCHO and isoprene emissions. 
  ...
  \subsection{Modelled impacts of reduced isoprene emissions}
    ...
    \subsubsection{Implications for ozone}
      ...
      \added[id=tb]{Figure \ref{BioIsop:results:emissions:fig_ozone_surf_summer} shows Australian summer surface level ozone with and without scaling isoprene emissions to match the multi-year top-down estimation.
      ...
      \mypic{Figures/OMI_link/new_emiss/O3_surf_map_Summer_05.png}{%
        \added[id=tb]{Top row: Summer surface-level ozone before (left) and after (right) scaling isoprene emissions in GEOS-Chem to match top-down estimates.
      ...
      \deleted[id=tb]{In future work, the estimated emissions will be downscaled to better evaluate their impacts on ozone in different NO$_x$ regimes (for instance over cities).}
      ...
      \added[id=tb]{Table \ref{BioIsop:results:new_emiss:tab_emissions_vs_ozone} shows regionally how the changed isoprene emissions affected ozone concentration yearly averages. 
...
%    
  ...
  \subsection{Top-down emissions}
    ...
    \end{equation*}
    Assuming each term is independent, \replaced[id=mh]{the following quadrature rules are used}{we use the following quadrature rules} to estimate random error in $\apost$:
    \begin{align}
    ...
    \end{align}
    Which leads to the uncertainty estimation for \replaced[id=mh]{the}{our} a posteriori emissions as follows
    \begin{align}
    ...
    For $\Delta{S}$ ($\Ogc = S \times \apri + \Omega_{OMI,0}$ from equation \ref{BioIsop:method:slope:eqn_isop_to_hcho})\replaced[id=mh]{, the variance in the monthly linear regression of modelled isoprene emissions and column HCHO is used}{ I use variance in the monthly linear regression of modelled isoprene emissions and column HCHO}, shown in Section \ref{BioIsop:uncertainty:Model}.
  ...
  \subsection{Model Uncertainty}
    ...
    \replaced[id=mh]{T}{Here I use the ratio of t}he upper bound of the 95\% confidence interval of the regression slope ($CI_{UB}$) \added[id=mh]{is used} to represent the relative uncertainty.
    \begin{equation}
...
\section{Conclusions and implications}
  ...
  % What I did
  In this chapter\deleted[id=mh]{, I created and tested} an isoprene emissions estimate (a posteriori) \added[id=mh]{is created and tested} based on OMI satellite measurements of HCHO, a high-yield product of isoprene oxidation.
...
%All model runs, configuration, and alterations were performed by the Author, with initial discussions with Jenny Fisher.
%Satellite products were downloaded, collated, and (where appropriate) recalculated by the author.
%Recalculation of the OMHCHO data product is in part based on code written by Paul Palmer and Randal Martin, with updates and usage instructions given by Luke Surl.
%Output created in this chapter that is used outside of this thesis includes a filtered and regridded (but not recalculated) version of OMHCHO used in \fullcite{Lieschke2019}, model run analyses used to debug GEOS-Chem satellite parameterisation, highly negative satellite pixels analysis that has been shared with the OMHCHO product developer, and products used in Chapter \ref{BioIsop} may form part of a publication in preparation: Greenslade J. W., Fisher J. A. et al. Top-down emissions of isoprene over Australia.
  \added[id=tb]{Model runs, data retrieval, analysis and error calculations and attribution in this Chapter are performed by the Author, with feedback and direction provided by Jenny Fisher and Clare Paton-Walsh.
